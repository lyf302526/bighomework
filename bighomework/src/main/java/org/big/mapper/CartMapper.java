...
public interface CartMapper{
	...
	@Insert("insert into cart(goods_id,image,price,user_id,count)values(#{goods_id},#{image},#{price},#{user_id},#{count})")
	int save(Cart cart);
	@Insert("insert into cart(goods_id,image,price,user_id)values(#{goods_id},#{image},#{price},#{user_id})")
	int add(Cart cart);
	...
	@Update("update cart set count=count-1 where goods_id=#{goods_id}")
	int reduceCart(@Param("goods_id") String goods_id);
}